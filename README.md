...
## How it works
This repository exists out of 2 permanent branches. ```main``` is my current live system and ```stage``` is my little playground for testing more extensive changes before publishing them to my website (f.e. test Yellow updates and new plugins, but also design and layout customizations). For creating new content, I only use the ```main``` branch.
